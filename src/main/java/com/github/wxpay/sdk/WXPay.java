...
public class WXPay {
    ...
    /**
     * 处理 HTTPS API返回数据，转换成Map对象。return_code为SUCCESS时，不需要验证签名。
     *
     * @param xmlStr API返回的XML格式数据
     * @return Map类型数据
     * @throws Exception
     */
    public Map<String, String> processResponseXmlWithNoValidate(String xmlStr) throws Exception {
        String RETURN_CODE = "return_code";
        String return_code;
        Map<String, String> respData = WXPayUtil.xmlToMap(xmlStr);
        if (respData.containsKey(RETURN_CODE)) {
     ...
     * 场景：沙箱环境
     *
     * @param reqData 向wxpay post的请求数据,不需要验证签名
     * @return API返回数据
     * @throws Exception
     */
    public Map<String, String> getSignKey(Map<String, String> reqData) throws Exception {
        ...
        String respXml = this.requestWithoutCert(url, this.fillRequestData(reqData), this.config.getHttpConnectTimeoutMs(),
                this.config.getHttpReadTimeoutMs());
        return this.processResponseXmlWithNoValidate(respXml);
    }
